...
export async function describeFrames(
  ...
  try {
    // 480px 宽 + q:v 4 下每帧约 30-60KB，8 帧请求体约 300-500KB，在接口限制内
    ...
    const timeHints = frames.map((f) => Math.round(f.time)).join("、");
    const promptText = `视频标题：${videoTitle ?? "未知"}
以下是从该视频不同时间点截取的 ${frames.length} 张画面（按时间顺序，对应秒数分别为：${timeHints}）。
...
输出 JSON，格式：{"frames":[{"time": 秒数, "description": "描述"}]}，frames 数组长度与输入图片数一致，按图片顺序排列。`;
    ...
    if (!response.ok) {
      throw new Error(`SiliconFlow 视觉 API 错误 (${response.status})`);
    }
    ...
    if (rawList) {
      ...
      frames.forEach((f, i) => {
        const item = rawList[i] as { description?: string; text?: string } | undefined;
        const desc = (item?.description || item?.text || "").trim();
        if (desc) descriptions.push({ time: f.time, description: desc });
      });
    } else {
      ...
      if (descriptions.length === 0) {
        ...
        if (typeof singleDesc === "string" && singleDesc.trim()) {
          descriptions.push({ time: frames[0].time, description: singleDesc.trim() });
        }
      }
    }
    if (descriptions.length === 0) {
      console.log("[vision] 响应未匹配预期结构:", JSON.stringify(parsed).slice(0, 200));
      return null;
    }
    ...
    console.log("[vision] 视觉理解失败，降级为音频-only:", err);
    return null;
  }
}
